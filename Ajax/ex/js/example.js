$(function(){                                            // DOM이 로드되면 실행한다
   ...
   $('#event a').click(function(e) {                     // 행사를 클릭했으면
      ...
      for (var i = 0; i < times[loc].length; i++) {      // 루프를 실행한다
         ...
         newContent += times[loc][i].title.replace(/ /g, '-') + '">'; // ID값 공백은 - 로 바꿔라
...
//descriptions.html#Intro-to-3D-Modeling
   ...
   //상단 네비게이션을 처리한다
   $('nav a').click(function(e){                          // 상단 메뉴를 클릭하면
      e.preventDefault();                                 // 페이지 이동을 취소한다
